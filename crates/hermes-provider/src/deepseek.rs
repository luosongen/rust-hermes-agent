...
/// DeepSeek 流式响应 Choice
#[derive(Debug, Deserialize)]
struct DeepSeekStreamChoice {
    delta: DeepSeekStreamContent,
    finish_reason: Option<String>,
}

/// DeepSeek 流式响应 Content
#[derive(Debug, Deserialize)]
struct DeepSeekStreamContent {
    role: Option<String>,
        ...
        while let Some(chunk) = stream.next().await {
            ...
            for line in text.lines() {
                if line.starts_with("data: ") {
                    ...
                    if let Ok(delta) = serde_json::from_str::<DeepSeekStreamDelta>(data) {
