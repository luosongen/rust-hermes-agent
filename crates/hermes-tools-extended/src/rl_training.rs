...
//!
//! 通过 tinker-atropos 提供 RLHF 训练能力。
...
impl RLTrainingTool {
    ...
    async fn start_training(&self, model: &str, dataset: &str) -> Result<String, ToolError> {
        ...
        tokio::spawn(async move {
            ...
            match cmd.spawn() {
                Ok(mut child) => {
                    ...
                    let status_path = training_dir_clone.join("status.json");
                    if let Ok(content) = tokio::fs::read_to_string(&status_path).await {
                        if let Ok(mut status) = serde_json::from_str::<TrainingStatus>(&content) {
                            status.state = if status_result.is_ok() && status_result.unwrap().success() {
                                TrainingState::Completed
                            } else {
                                TrainingState::Failed
                            };
                            status.updated_at = crate::rl_training::RLTrainingTool::now();
                            let _ = tokio::fs::write(&status_path, serde_json::to_string_pretty(&status).unwrap()).await;
                        }
                    }
                }
                Err(e) => {
    ...
    async fn get_status(&self, training_id: &str) -> Result<TrainingStatus, ToolError> {
        self.read_status(training_id).await
    }
    ...
    async fn list_trainings(&self) -> Result<Vec<TrainingStatus>, ToolError> {
        ...
        trainings.sort_by(|a, b| b.started_at.partial_cmp(&a.started_at).unwrap_or(std::cmp::Ordering::Equal));
...
impl Tool for RLTrainingTool {
    ...
    fn description(&self) -> &str {
        "Reinforcement learning training tool using tinker-atropos. Actions: start_training, get_status, list_trainings, cancel_training, get_results"
    }
    ...
    fn parameters(&self) -> serde_json::Value {
        json!({
            "type": "object",
            "oneOf": [
                {"properties": {"action": {"const": "start_training"}, "model": {"type": "string"}, "dataset": {"type": "string"}}, "required": ["action", "model", "dataset"]},
    ...
    async fn execute(&self, args: serde_json::Value, _context: ToolContext) -> Result<String, ToolError> {
        ...
        enum TrainingAction {
            StartTraining { model: String, dataset: String },
        ...
        match params {
            TrainingAction::StartTraining { model, dataset } => {
                let id = self.start_training(&model, &dataset).await?;
